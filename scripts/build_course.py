...
SUBTHEME_ESSAYS = {
    ...
    "invariant-receipts": [
        ...
        "From first principles, the problem is trust. A picture can move so much that the reader loses track of why the answer should still be the same. The invariant says which part of the object came through the motion unchanged. It lets the proof replace visual resemblance with accountable evidence.",
        ...
        "The reader's check is whether the receipt is matched to the move. A count that changes during a legal redraw is not an invariant for that problem. A count that survives every legal redraw can speak for all of them.",
    ],
    "holes-and-boundaries": [
    ...
    "mechanisms-and-locks": [
        ...
        "The hard part is honest modeling. The state space must include real freedoms and remove real forbidden states. If a joint can rotate but the model forgets that rotation, the model may invent a lock. If collisions are forbidden but the model allows them, the model may erase a real obstruction. The topology only speaks for the system after the translation is right.",
...
FAMILY_ESSAYS = {
    ...
    "motion-family": [
        ...
        "The detail that keeps the method honest is modeling. The state space must contain the right freedoms and remove the right forbidden states. If that translation is wrong, the topological conclusion may be true about the model but false about the physical situation. Modeling is therefore part of the proof, not a story attached before the proof begins.",
    ],
}
