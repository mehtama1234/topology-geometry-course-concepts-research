...
FAMILY_DEPTH = {
    "deformation-family": {
        ...
        "how_it_works": "A deformation proof has four parts: the starting object, the contract of allowed motion, the protected feature, and the simpler endpoint. The proof fails if any part is vague. The endpoint is convincing only because the route to it was legal and carried the protected feature all the way.",
...
FAMILY_CONTRACTS = {
    "deformation-family": {
        "input": "A hard picture whose exact drawing is less important than a route, boundary, side behavior, gluing instruction, forbidden crossing, or count it carries. The input must include the data that are not allowed to move.",
        "action": "State the legal moves, move the object through only those moves, and replace the hard picture by an easier one. Each move has to preserve the named question, not merely make the drawing cleaner.",
        "evidence": "The protected evidence is the fact that survives the whole motion: same endpoints, same boundary data, same route relation, same gluing, same side behavior, or same invariant. That evidence is why the final picture can speak for the first.",
    ...
    "counting-family": {
        "input": "A situation where visible local events change too easily: cells split, crossings appear, defects move, arrows turn, or raw counts fluctuate. The input needs both a local event and a list of the legal ways that event can change.",
        "action": "Design a count that ignores harmless changes by cancellation, alternating terms, parity, or signs with geometric meaning. The design must match the legal local changes, otherwise the count is only a statistic of one drawing.",
        "evidence": "The protected evidence is the total that legal redrawings cannot change, even though individual local events may appear or disappear. The total remembers forced structure rather than clutter because the harmless changes have already been audited.",
    ...
    "motion-family": {
        "input": "A physical or dynamical situation whose direct motion is too complicated to follow point by point. The input must be translated into complete states, not vague physical impressions or a picture that forgets a real degree of freedom.",
        "action": "Build the space of possible states, translate motion into paths or rules, and apply fixed-point, index, or obstruction reasoning there. The action moves from behavior to the shape of possibility, while preserving the original freedoms and forbidden states.",
        "evidence": "The protected evidence is the shape of possibility: a barrier, hole, forced self-agreement, index total, or missing legal path. That evidence constrains behavior without solving every trajectory because each point still means a real possible state.",
...
FAMILY_PLAYBOOKS = {
    "deformation-family": {
        "setup": "Start with the original object, not the prettier target picture. Mark the fixed data: endpoints, boundary pieces, obstacles, gluing rules, side choices, forbidden crossings, and the question being protected.",
        "move": "Move the object continuously through only the allowed changes, checking after each kind of move that the protected fact has not changed. The route is part of the proof and must be specific enough to audit.",
    ...
    "counting-family": {
        "setup": "Start by listing the local events that may change under motion: cells split, crossings appear, turns shift, signs reverse, or defects move. The list tells the count what it must survive.",
    ...
    "motion-family": {
        "setup": "Start by listing what information describes one complete state: positions, angles, boundary conditions, contact rules, forbidden collisions, and any rule acting on states. One point should mean one possible situation.",
        "move": "Turn those states into a space, read motion as paths or maps, and then apply deformation, fixed-point, index, or obstruction reasoning inside that state space. The proof happens in the model, so the model must keep the original freedoms and restrictions.",
