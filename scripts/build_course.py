...
FAMILY_PLAYBOOKS = {
    "deformation-family": {
        "setup": "Start with the original object, not the cleaned-up target picture. Mark the fixed data before any simplification: endpoints, boundary pieces, obstacles, gluing rules, side choices, forbidden crossings, and the question being protected. If the fixed data are unclear, the motion has no proof value.",
        "move": "Move the object continuously through only the allowed changes. Check the in-between states, not only the start and finish. After each kind of move, say why the protected fact has not changed and why the move did not cross a forbidden event.",
        "payoff": "The final simpler picture answers the original problem because the legal route to it carried the same question all the way across. The simplified picture is evidence only through that route; without the route, it is just another drawing.",
        "failure": "The method fails when the easy picture is reached by crossing through a forbidden object, moving fixed data, skipping intermediate states, or silently changing what sameness means. Then the final picture may be readable but it answers a different question.",
        "reader_test": "Can the reader describe the full legal route, name one tempting illegal shortcut, and state what fact was preserved along the route at every step?",
    },
    "counting-family": {
        "setup": "Start by listing the local events that may change under motion: cells split, crossings appear, turns shift, signs reverse, or defects move. The list tells the count what it must survive and prevents the reader from counting whatever is easiest to see.",
        "move": "Choose a count whose allowed changes cancel by design, using alternating terms, parity, or signs that come from the geometry of the situation. Then test the count on each basic local change before using it globally.",
        "payoff": "Once the protected total is known in one honest version, every legally related version must obey it. Impossibility or forced existence can follow without checking every drawing because the local survival test already covers the redrawings.",
        "failure": "The method fails when the count changes under a harmless legal redraw, when signs are assigned without a checkable direction rule, or when the count is treated as evidence before its survival has been tested. Then the arithmetic follows the picture too closely.",
        "reader_test": "Can the reader explain one local change, show why the chosen count stays fixed through that change, and state what conclusion the count can force for the whole problem?",
    },
    "surface-family": {
        "setup": "Start by naming the whole surface and its edge behavior: boundary, handles, crosscaps, holes, orientation, and any gluing instructions. The carrier surface is part of the data, not background scenery.",
        "move": "Break the surface into manageable pieces, track local information on the pieces, then glue the account back together without losing global restrictions. The recombination is where the surface speaks, because that is where local choices must agree or fail to agree.",
        "payoff": "The conclusion belongs to the whole surface, not to one patch, one drawing, or one chosen decomposition. It explains what local choices must do when completed, such as which totals, side behaviors, or boundary contributions cannot be avoided.",
        "failure": "The method fails when local ordinariness is mistaken for global simplicity, when boundary and orientation data are dropped, or when a cell count is made before the identifications are understood. Small-patch evidence cannot replace the whole surface.",
        "reader_test": "Can the reader point to the full trip, full gluing, or full sum where the global surface constraint appears, and state exactly what the local view missed?",
    },
    "embedding-family": {
        "setup": "Start by separating the required object from the surrounding room: which points connect, which loops link, which crossings are forbidden, and what surface or space carries the placement. The room is part of the problem because different rooms permit different routes.",
        "move": "Try legal placements while tracking route relations, over-under information, crossings, winding, and endpoint order that cannot be erased by allowed motion. The move tests the placement relation, not drawing skill or the neatness of one sketch.",
        "payoff": "The result is either a legal placement or a proof that every legal placement must encounter the same obstruction. A real obstruction defeats all legal attempts, not only the first drawing that comes to mind.",
        "failure": "The method fails when one bad drawing is treated as proof, when spatial information such as over-under order is lost in a flat picture, or when the surrounding surface is quietly changed. The missing data may be the whole point.",
        "reader_test": "Can the reader say why every legal attempt is blocked, rather than only why one attempted drawing failed, and name the placement relation that survives redrawings?",
    },
    "motion-family": {
        "setup": "Start by listing what information describes one complete state: positions, angles, boundary conditions, contact rules, forbidden collisions, and any rule acting on states. One point should mean one possible situation, not a partial sketch of it.",
        "move": "Turn those states into a space, remove forbidden states, read motion as paths or maps, and then apply deformation, fixed-point, index, or obstruction reasoning inside that state space. The proof happens in the model, so the model must keep the original freedoms and restrictions.",
        "payoff": "The original system inherits the conclusion because the state space records the real freedoms and restrictions of the physical or dynamical problem. The result is behavioral, not only geometric: a blocked path, forced state, equilibrium, or unavoidable defect.",
        "failure": "The method fails when the state space omits a real freedom, adds a false wall, keeps forbidden states inside, or cannot translate its conclusion back to the original motion.",
        "reader_test": "Can the reader name the state space, the forbidden states, the rule or path, the topological feature forcing behavior, and the physical conclusion that follows?",
    },
}
