...
SOURCE_NUANCE_REPAIRS = [
    ...
    },
    {
        ...
        "do_not_claim": "Do not let product or quotient appear as unexplained vocabulary. A beginner should see the independent choices recorded or the identifications imposed before the formal word is allowed to shorten the sentence.",
...
THEME_ESSAYS = {
    ...
    "count-what-survives": [
        ...
        "A reader should leave this theme asking not 'what can I count?' but 'what count is protected by the rules of this problem?' Signs and alternating terms are not formal decoration. They are repair mechanisms for counts that would otherwise be too brittle. The surviving total is the mathematical evidence.",
    ],
    "local-to-global": [
    ...
    "generic-before-exception": [
        ...
        "Stable events can be assigned stable evidence. The exception is not ignored; it is handled through the ordinary cases it connects. A reader should ask what tiny legal nudge would reveal, what ordinary events appear on either side of the nudge, and what invariant remains through that nudge. That question turns a special picture into a before-and-after audit rather than a new topic.",
    ],
    "pictures-to-proofs": [
...
CONCEPT_DEPTH = {
    ...
    "invariant": {
        ...
        "beginner_trap": "The trap is to expect one invariant to tell the whole story. Most invariants are partial witnesses. A single preserved difference can prove impossibility, even when it cannot classify every possible shape or explain every similarity.",
    ...
    "euler-characteristic": {
        ...
        "beginner_trap": "The trap is to memorize the formula without seeing the cancellation. The formula survives refinement, so it belongs to the surface rather than to one drawing of the surface or one chosen mesh of pieces.",
    ...
    "fixed-points": {
        ...
        "beginner_trap": "The trap is to expect a fixed-point theorem to compute the fixed point. These theorems often prove existence only, which is already enough when exact computation is unavailable or when the rule is known only qualitatively.",
...
CONCEPT_ESSAYS = {
    ...
    "invariant": [
        ...
        "The beginner trap is to treat any number attached to a picture as an invariant. A raw crossing count is not protected if legal motion can create or remove a canceling pair. A length is not protected if the object can stretch. The invariant is the part of the evidence that the allowed moves cannot destroy.",
    ],
    "topology-vs-geometry": [
...
LECTURE_ESSAYS = {
    ...
    10: [
        ...
        "For a beginner, the hard part is accepting that existence can be a geometric consequence. The theorem is not guessing that a fixed point probably exists. It says the whole filled shape leaves no continuous escape plan. The closed ball supplies the trap: it contains the interior and the boundary together, and a continuous rule must move nearby points in compatible ways while staying inside that same filled shape.",
        "The lecture also shows why changing the space changes the claim. A closed ball, an open ball, a circle, and a torus do not give a continuous map the same constraints. The theorem's force comes from the exact shape of the domain and the rule that points remain inside it. A beginner should therefore read the hypotheses as the mechanism, not as formal decoration: closed, filled, same space, inside itself, and no jumps are the pieces that block the escape plan.",
...
def build_concept_self_check(concept):
    ...
    failure_prompt = varied((cid, "failure-check-prompt"), [
        f"Say where the idea fails before trusting the conclusion: {failure} Then name the beginner mistake it prevents: {beginner_trap} {failure_close}",
        ...
        f"State the condition that would make the claim stop working: {failure} Then connect that break to the beginner trap: {beginner_trap} {failure_close}",
        f"Give the reader a failure scene: {failure} The beginner trap explains why that scene is tempting: {beginner_trap} {failure_close}",
        f"Name the nearest false use of the idea: {failure} Then state the trap it exposes: {beginner_trap} {failure_close}",
        f"Check the boundary of the concept: {failure} The beginner trap marks the shortcut that crosses that boundary: {beginner_trap} {failure_close}",
        f"Say what assumption cannot be dropped: {failure} Then show how the beginner trap drops or blurs it: {beginner_trap} {failure_close}",
...
<section class="lecture">
  ...
  <p>The wrong-transfer line catches a common beginner mistake: transferring the visible story instead of the mathematical job. A useful comparison does not say only that two examples look alike; it says what object, move, and protected fact are the same kind of thing.</p>
</section>
...
<section class="lecture">
  ...
  <p>Use the repaired sentence on the matching lecture, concept, theorem contract, or source page. A reader should be able to return to the original explanation and see exactly where the missing condition now belongs.</p>
</section>
<section class="lecture">
...
<section class="lecture">
  ...
  <p>The do-not-claim line is a guardrail for plain language. It keeps everyday wording honest: the page should be simple enough for a beginner, but still careful about what was actually supported by recovered captions and visible course structure.</p>
</section>
