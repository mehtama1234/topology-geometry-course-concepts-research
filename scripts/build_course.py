...
CONCEPT_CONTRASTS = [
    ...
    },
    {
        ...
        "bridge": "Many models start with a product and become a configuration space only after the real restrictions are added. The topology speaks about the restricted state space, not the larger wishful one. This is why collisions, locked positions, or missing rotations cannot be treated as afterthoughts.",
...
CHANGE_LEDGER_ROWS = [
    ...
    },
    {
        ...
        "why_matters": "Shape can force an agreement even when calculation does not find it. That is why the course can prove that some self-match must occur after checking the rule's contract.",
...
CONCEPT_ESSAYS = {
    ...
    "triangulation": [
        ...
        "The triangulation is scaffolding, not the surface fact itself. A different triangulation should not change the topological conclusion. If the answer depends on the exact mesh, the count has not found a surface fact. This is why triangulation supports Euler characteristic and surface classification: it lets a soft object enter exact reasoning, while later cancellation proves that the reasoning did not depend on one arbitrary set of divisions.",
    ...
    "winding-linking": [
        ...
        "Linked strip demonstrations train the eye to see this relationship. The pieces may flex, twist, and move, but the question is whether legal motion can separate their routes. If separation requires a forbidden pass-through, the linking is evidence. This is why the demonstration belongs in the same course as intersection number rather than only as a physical puzzle.",
...
MATH_WHY = {
    ...
    "important_detail": "The allowed changes are the whole contract. A result is only as strong as the promise that the change did not cut, glue, pass through a forbidden object, erase a boundary, reverse a side rule, omit a state variable, or invent a sign convention. This is why the course spends so much time on ordinary position, orientation, boundaries, and signs. Those details are the checks that keep a short explanation from silently changing the problem.",
...
LECTURE_EXAMPLES = {
    ...
    15: [
        ...
        {"title": "Pictures earn their role by carrying constraints", "text": "The course's pictures show what can move, what cannot move, and what count survives. That is why the final summary ties the visual style to mathematical force.", "concepts": ["duality", "generic-position", "euler-characteristic"]},
...
LECTURE_SOURCE_LENS = {
    ...
    4: [
        ...
        "This lecture should make later pages less abstract. When a map, field, or route lives on a manifold, its behavior is controlled by these hidden travel rules. The drawing is useful only after the identification rule has been read. That is why orientation and quotient space belong together: the way a traveler returns decides whether local direction choices can agree globally.",
    ],
    5: [
