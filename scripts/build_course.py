...
THEME_ESSAYS = {
    ...
    "local-to-global": [
        ...
        "The course-level payoff is prediction from whole shape. A surface can force a total count. A sphere can demand a vector-field defect. A boundary can add a term that local interior reasoning would miss. A state space can block a motion because its possible paths do not connect in the needed way. Local work supplies the pieces; global structure decides whether the pieces can coexist.",
        "For a beginner, the key is to stop asking only whether each small neighborhood looks possible. The harder question is whether all the local choices can be made consistently at once. The companion should keep pointing to the full journey, full sum, or full gluing step where the whole object finally speaks.",
    ],
    "generic-before-exception": [
        ...
        "The course-level payoff is that exceptions become controlled transitions rather than mysterious failures. A singular moment is studied through the ordinary pictures around it. The proof asks what appeared, what disappeared, whether signs cancel, and whether the protected total survived. This is how the course handles motion without letting rare perfect alignments dominate the argument.",
        "The mathematical principle is that stable events can be assigned stable evidence. The exception is not ignored; it is handled through the ordinary cases it connects. A reader should ask what tiny legal nudge would reveal and what invariant remains through that nudge.",
    ],
    "pictures-to-proofs": [
    ...
    "shape-as-machine": [
        ...
        "The course-level payoff is practical. A fixed-point theorem can prove an unavoidable self-agreement without locating it. Poincare-Hopf can demand defects before solving the differential equation. A configuration space can show why a mechanism cannot move from one state to another. These conclusions come from the shape of possibility, not from step-by-step prediction.",
...
SUBTHEME_ESSAYS = {
    ...
    "turning-and-curvature": [
        ...
        "The important mathematical move is summing local behavior only after knowing what the sum means. Turning around one defect gives an index. Curvature over a surface gives a total only after boundary and corner effects are handled. A local contribution is useful only when the full account includes every place where turning can enter.",
    ...
    "singular-moments": [
        ...
        "The useful question is not whether singular moments occur. They often do during motion. The useful question is what they are allowed to change. If the event creates opposite signs or preserves parity, it cannot destroy the invariant the proof is using. If it changes the protected total, then the supposed invariant was not protected after all.",
...
FAMILY_ESSAYS = {
    ...
    "surface-family": [
        ...
        "The mathematical principle is that local freedom is not the same as global freedom. A patch may allow an arrow, a side choice, or a sign, while the completed surface refuses to let all those local choices agree at once. Surface bookkeeping is how the course finds the refusal.",
    ],
    "embedding-family": [
...
MATH_WHY = {
    ...
    "principle": "Topology and geometry become decisive when local details are organized so that the whole shape has fewer choices than it appears to have. Local patches may look free. Local arrows may look adjustable. Local crossings may look removable. But the completed object can force a side reversal, a nonzero count, a fixed point, or a defect. The mathematical principle is that a named object plus a named rule of change can turn many small freedoms into one unavoidable conclusion.",
...
PROOF_MOVES = [
    ...
    },
    {
        ...
        "why": "A useful count is designed around its allowed changes. Euler characteristic, parity, intersection number, and index all work because local bookkeeping changes either cancel, arrive in pairs, or preserve a signed total.",
